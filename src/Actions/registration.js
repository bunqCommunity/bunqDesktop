...
import { loadStoredAccounts } from "~store/accounts/thunks";
import { loadStoredBunqMeTabs } from "~store/bunqMeTabs/thunks";
...
import { loadStoredContacts } from "~store/contacts/thunks";
...
export function registrationLogin(apiKey = false, deviceName = false, environment = false, permittedIps = []) {
    ...
    return async dispatch => {
        ...
        if (!apiKey) {
            dispatch(applicationActions.setStatusMessage(statusMessage));
            if (BunqDesktopClient.derivedPassword) {
                try {
                    ...
                    dispatch([
                        snackbarActions.open(failedToDecryptDataMessage),
                        registrationLogOut(false),
                        registrationNotLoading(),
                        registrationSetNotReady(),
                    ]);
        ...
        try {
            ...
            dispatch(applicationActions.setStatusMessage(statusMessage2));
            await BunqDesktopClient.BunqJSClientInstall();

            dispatch(applicationActions.setStatusMessage(statusMessage3));
            await BunqDesktopClient.BunqJSClientRegisterDevice();

            dispatch(applicationActions.setStatusMessage(statusMessage4));
            ...
            BunqErrorHandler(batchedActions, exception, false, BunqJSClient);
            dispatch(batchedActions.concat([
        ...
        setTimeout(() => {
            dispatch([
                applicationActions.setStatusMessage(""),
                registrationNotLoading(),
    ...
    return dispatch => {
        ...
        BunqDesktopClient.switchStoredApiKey(keyIndex)
            ...
            .catch(error => {
                ...
                dispatch(snackbarActions.open({ message: failureMessage }));
            });
    };
};
