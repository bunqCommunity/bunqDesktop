...
export default (batchedActions: BatchedActions, error, customError: boolean | string = false, BunqJSClient = false) => {
    ...
    if (error.toString() === "Error: Network Error") {
        ...
        batchedActions.push(snackbarActions.open(offlineError));

        // enable offline mode
        batchedActions.push(applicationActions.setOffline());
        return;
    }
    ...
    if (error.errorCode) {
        switch (error.errorCode) {
            ...
            case ErrorCodes.INVALID_RESPONSE_RECEIVED:
                return defaultErrorMessage(batchedActions, invalidResponseError);
        }
    }

    // fallback to a default message
    if (!response) {
        return defaultErrorMessage(batchedActions, customError);
    }
